...
import { composeReleaseNotes, extractVNextSection } from './release-compose-notes.mjs';
...
test('composeReleaseNotes produces one section per package with non-empty vNext', () => {
  ...
  try {
    ...
    strictEqual(
      notes,
      ['## @next-model/core', '', '- Big change', '', '## @next-model/zod', '', '- Zod change', ''].join('\n'),
    );
  } finally {
...
test('composeReleaseNotes skips private packages', () => {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: '@next-model/demo', version: '1.0.0-rc.0', private: true }, null, 2)}\n`,
  );
  writeFileSync(join(pkgDir, 'HISTORY.md'), '# History\n\n## vNext\n\n- private change\n\n## v0.0.1\n');
  try {
    strictEqual(composeReleaseNotes(root), '');
  } finally {
    rmSync(root, { recursive: true, force: true });
